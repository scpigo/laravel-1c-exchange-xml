...
class GenerateService extends ExchangerAbstract implements GenerateInterface {
    public function generate() {
        $data = $this->getArray();

        $array = Fractal::create()
            ->item($data, new OrderXmlTransformer())
            ->serializeWith(new ArraySerializer())
            ->toArray();

        $xml = ArrayToXml::convert($array, XmlExchangeFields::DOCUMENT, false, 'UTF-8');
